...
try:
    from src.features.passiveSRC.rules_for_all_passives import create_matcher
except:
...
class PassivePyAnalyzer:
    ...
    def __init__(
        self, nlp: spacy.language.Language = None, spacy_model: str = "en_core_web_lg"
    ):
        """
        Create the Detector
    ...
    def parse_sentence(self, sentence):
        ...
        df_output = pd.DataFrame(index=["POS", "dependency", "tag", "lemma", "moprh"])

        for token in doc:
            df_output.loc[:, token.text] = [
                token.pos_,
                token.dep_,
                token.tag_,
                token.lemma_,
    ...
    def _detect_sents(self, document, batch_size, n_process):
        ...
        m = 0
        for record_doc in self.nlp.pipe(
            document, batch_size=batch_size, n_process=n_process
        ):
            sentences = list(record_doc.sents)
            sentences = [
                str(sentence) if len(sentence) >= 2 else "Not a Sentence"
            ...
            for sentence in sentences:
                ...
                if i != 0:
                    if (
                        (
                            re.search(r"^ *mais", sentence)
                            and not re.search(r".$", sentences[i - 1])
                        )
                        or all(
                            (
                ...
                if i != len(sentences) - 1:
                    if re.search(
                        r", *$", sentence
                    ):  # remove the one that's ended with comma
        ...
        return np.array(count_sents, dtype="object"), np.array(
            all_sentences, dtype="object"
        )
    ...
    def match_sentence_level(
        self, df, column_name, n_process=1, batch_size=1000, add_other_columns=True
    ):
        """
        Parameters
    ...
    def match_corpus_level(
        ...
        df_output = self.match_sentence_level(
            df, column_name, n_process, batch_size, add_other_columns
        )
