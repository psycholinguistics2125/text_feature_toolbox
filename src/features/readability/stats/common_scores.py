...
def GFI_score(text, statistics=None):
    ...
    if statistics is not None:
        return 0.4 * (
            (statistics["totalWords"] / statistics["totalSentences"])
            + 100 * statistics["totalLongWords"] / statistics["totalSentences"]
        )
    totalWords = 0
    ...
    score = 0.4 * (
        (totalWords / totalSentences) + 100 * totalLongWords / totalSentences
    )
    return score
...
def ARI_score(text, statistics=None):
    ...
    if statistics is not None:
        return (
            4.71
            * (
                (statistics["totalCharacters"] / statistics["totalWords"])
                + 0.5 * statistics["totalWords"] / statistics["totalSentences"]
            )
            - 21.43
        )
    totalWords = 0
    ...
    score = (
        4.71 * ((totalCharacters / totalWords) + 0.5 * totalWords / totalSentences)
        - 21.43
    )
    return score
...
def FRE_score(text, statistics=None):
    ...
    if statistics is not None:
        return (
            206.835
            - 1.015 * (statistics["totalWords"] / statistics["totalSentences"])
            - 84.6 * (statistics["totalSyllables"] / statistics["totalWords"])
        )
    totalWords = 0
    totalSentences = len(text)
    totalSyllables = 0
...
def FKGL_score(text, statistics=None):
    ...
    if statistics is not None:
        return (
            0.39 * (statistics["totalWords"] / statistics["totalSentences"])
            + 11.8 * (statistics["totalSyllables"] / statistics["totalWords"])
            - 15.59
        )
    totalWords = 0
    totalSentences = len(text)
...
def SMOG_score(text, statistics=None):
    ...
    if statistics is not None:
        return (
            1.043
            * math.sqrt(
                statistics["nbPolysyllables"] * (30 / statistics["totalSentences"])
            )
            + 3.1291
        )
    totalSentences = len(text)
    ...
    for sent in text:
        nbPolysyllables += sum(
            utils.syllablesplit(word) for word in sent if utils.syllablesplit(word) >= 3
        )
...
def REL_score(text, statistics=None):
    ...
    if statistics is not None:
        return (
            207
            - 1.015 * (statistics["totalWords"] / statistics["totalSentences"])
            - 73.6 * (statistics["totalSyllables"] / statistics["totalWords"])
        )
    totalWords = 0
    totalSentences = len(text)
    totalSyllables = 0
    ...
    score_REL = (
        207
        - 1.015 * (totalWords / totalSentences)
        - 73.6 * (totalSyllables / totalWords)
    )
    return score_REL
