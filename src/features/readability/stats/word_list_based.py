...
def average_levenshtein_distance(df, token_list, nlp=None, mode="old20"):
    """
    ...
    clean_token_list = [
        elt.lower() for elt in token_list if elt not in [c for c in string.punctuation]
    ]
    clean_text = " ".join(clean_token_list)

    # Get each word and count the number of times it appears
    counter = Counter(clean_token_list)
    average_value = 0
    ...
    if mode == "old20":
        ...
        raise ValueError(
            "Parameter 'type' with value",
            mode,
            "is not supported, please provide one of the following values instead : 'old20', 'pld20'",
        )
...
def dubois_buyse_ratio(df, lemma_list, nlp=None, typ="total", filter=None):
    """
    ...
    clean_lemma_list = [
        elt.lower() for elt in lemma_list if elt not in [c for c in string.punctuation]
    ]
    clean_text = " ".join(clean_lemma_list)

    # Converting each recognized noun in text to its lemma in lowercase, in order to check if there's a match
    noun_counter = Counter(clean_lemma_list)
    total_words = 0
    ...
    if typ != "total":
        if not isinstance(filter, (int, tuple, list)):
            raise TypeError(
                "Type of parameter 'filter' cannot be '",
                type(filter),
                "', needs to be int, tuple, or list",
            )
        ...
        if typ == "echelon":
            if isinstance(filter, int):
                if not (1 <= filter <= 43):
                    raise ValueError(
                        "Value of parameter 'filter' cannot be less than 1 or more than 43 when using parameter 'echelon'"
                    )
            ...
            if isinstance(filter, int):
                if not (6 <= filter <= 15):
                    raise ValueError(
                        "Value of parameter 'filter' cannot be less than 6 or more than 15 when using parameter 'age'"
                    )
            ...
            if isinstance(filter, int):
                if not (2 <= filter <= 5):
                    raise ValueError(
                        "Value of parameter 'filter' cannot be less than 2 or more than 5 when using parameter 'cycle'"
                    )
            ...
            raise ValueError(
                "Parameter 'type' with value",
                typ,
                "is not supported, please provide one of the following values instead : 'total', 'echelon', 'age', or 'cycle'",
            )
    ...
    for element in noun_counter:
        ...
        # number_words += 1
        total_words_in_list += (
            noun_counter[element] if element in df["Mot"].values else 0
        )
