...
def convert_text_to_string(text):
    ...
    * If the text is composed of sentences, that have not been split into tokens, the function takes care of concatening sentences.
    ...
    if isinstance(text, str):
        ...
        doc = (
            " ".join(text[0][:-1]) + text[0][-1]
        )  # Make first sentence not start with a whitespace, and remove whitespace between text and last punctuation mark.
        if len(text) > 1:
            for sentence in text[1:]:
                if len(sentence) > 0:
                    doc = (
                        doc + " " + " ".join(sentence[:-1]) + sentence[-1]
                    )  # Remove whitespace between text and last punctuation mark.
...
def count_occurences_in_document(text, spacy_filter, nlp=None, mode="text"):
    ...
    raise TypeError(
        "Type of parameter 'mode' cannot be '",
        type(mode),
        "', needs to be 'text', or 'sentence'",
    )
...
def load_dependency(dependency_name, nlp_processor=None):
    ...
    if dependency_name == "GPT2_LM":
        ...
        try:
            with open(
                os.path.join(DATA_ENTRY_POINT, "corpus_fauconnier.bin"), "rb"
            ) as f:
                model = KeyedVectors.load_word2vec_format(
                    os.path.join(DATA_ENTRY_POINT, "corpus_fauconnier.bin"),
                    binary=True,
            ...
            print(
                "WARNING : Acquiring french word2vec model remotely since model was not found locally."
            )
            response = requests.get(url)
            with open(
                os.path.join(DATA_ENTRY_POINT, "corpus_fauconnier.bin"), "wb"
            ) as f:
    ...
    # These depend on actual data to be initialized, so i'll do it when I clean up BERT/fastText.
        ...
        raise ValueError(
            "Dependency '",
            dependency_name,
            "' was not recognized as a valid dependency.",
        )
